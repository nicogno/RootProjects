{
    ...
    TCanvas *c1 = new TCanvas("c1","Spettro_Sodio_Coincidenze_-25_gradi_300_secondi", 1600, 1200);
    TH1D *h1 = new TH1D("Histogram and Fit parameters","Coincidenze Sodio (-25 gradi, 300 secondi);# CH;Counts",1024,0,1023);
    
    in.open("Spettro_Sodio_Coincidenze_-25_gradi_300_secondi.mca");
    ...
    TF1 *g0    = new TF1("g0","gaus",620,660);//638,672);
    TF1 *g1    = new TF1("g1","gaus",661,720);
    ...
    h1->Fit(total,"RB");
    c1->SaveAs("Spettro_Sodio_Coincidenze_-25_gradi_300_secondi.pdf",".pdf");
    c1->SaveAs("Spettro_Sodio_Coincidenze_-25_gradi_300_secondi.root",".root");
    
    /*
    TF1 *totalTest = new TF1("totalTest","gaus",60,100);
     
     for(int a = 620; a < 655; a++) {
         for(int b = 672; b > a; b--) {
             if(b > 655 and b-a >30) {
                 h1->Fit("totalTest", "MQ", "", a, b);
                 if(totalTest->GetParameter(1) > 654 && totalTest->GetParameter(1) < 656){
