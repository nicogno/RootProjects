{
    ...
    TCanvas *c1 = new TCanvas("c1","Spettro_Sodio_Coincidenze_-35_gradi_300_secondi", 1600, 1200);
    TH1D *h1 = new TH1D("Histogram and Fit parameters","Coincidenze Sodio (-35 gradi, 300 secondi);# CH;Counts",1024,0,1023);
    
    in.open("Spettro_Sodio_Coincidenze_-35_gradi_300_secondi.mca");
    ...
    TF1 *g0    = new TF1("g0","gaus",638,710);
    TF1 *total = new TF1("total","pol0(0)+gaus(1)",0,1000);
    total->SetLineColor(2);
    total->SetLineWidth(3);
    total->SetLineStyle(10);
    h1->Fit(g0,"R","goff");
    h1->Fit(background,"RB+","goff");
    background->GetParameters(&par[0]);
    g0->GetParameters(&par[1]);
    total->SetParameters(par);
    total->SetParNames("Background","Constant","Mean", "Sigma");
    h1->Fit(total,"R");
    c1->SaveAs("Spettro_Sodio_Coincidenze_-35_gradi_300_secondi.pdf",".pdf");
    c1->SaveAs("Spettro_Sodio_Coincidenze_-35_gradi_300_secondi.root",".root");
}
