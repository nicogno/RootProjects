{
    ...
    TCanvas *c2 = new TCanvas("c2","Spettro_Sodio_Acciaio_No_Fit", 1600, 1200);
    TH1D *h1 = new TH1D("Histogram and Gauss Fit parameters","Spettro Sodio (scintillatore A);# CH;Counts",1024,0,1023);
    TH1D *h2 = new TH1D("Histogram parameters","Spettro Sodio (scintillatore A);# CH;Counts",1024,0,1023);
    TH1D *h3 = new TH1D("Gauss Fit parameters","",1024,0,1023);
    
    in.open("Spettro_Sodio_Acciaio.mca");
    ...
    h2->Draw();
    c2->SaveAs("Spettro_Sodio_Acciaio_No_Fit.pdf",".pdf");
    
    TCanvas *c1 = new TCanvas("c1","Spettro_Sodio_Acciaio", 1600, 1200);
    ...
    TF1 *fit0 = new TF1("fit0","gaus",180,222); //Chi quadro critico 55.76  (195,238)
    fit0->SetParNames("Constant","Mean","Sigma");
    TF1 *g1 = new TF1("g1","gaus",327,366);
    fit0->SetLineColor(6);
    g1->SetLineColor(2);
    g1->SetLineWidth(5);
    g1->SetLineStyle(10);
    ...
    TF1 *totalTest = new TF1("totalTest","gaus",80,140);
     ...
     for(int a = 0; a < 200; a++) {
         for(int b = 250; b > a; b--) {
             if(a > 150 && b<300 && b-a>30 && b>230) {
                 h1->Fit("totalTest", "MQ", "", a, b);
                 if(totalTest->GetChisquare()/totalTest->GetNDF() < 2.7 && totalTest->GetParameter(1)+totalTest->GetParameter(2) > 220 && totalTest->GetParameter(1)+totalTest->GetParameter(2) < 229){
                 cout << "a = \t" << a << "\t b = \t" << b << "\t Chi Quadrato \t " << totalTest->GetChisquare() << "\t Mean + Sigma = \t" << totalTest->GetParameter(1)+totalTest->GetParameter(2) << "\t b-a \t" << b-a << "\t DOF \t" << totalTest->GetNDF() << "\t ChiSquare/DOF \t" <<totalTest->GetChisquare()/totalTest->GetNDF() << endl;
                 }
             }
         }
     }

    totalTest->SetLineColor(2);
    totalTest->SetLineWidth(5);
    
    c1->SaveAs("Spettro_Sodio_Acciaio_Con_Fit.pdf",".pdf");
    c1->SaveAs("Spettro_Sodio_Acciaio.root",".root");
}
